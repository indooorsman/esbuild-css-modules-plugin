  ...
  if (options.inject) {
    ...
    const injectorCode =
      typeof options.inject === 'function'
        ? `const inject = () => {
if (globalThis["__${digestPlaceholder}_injected"]) { return; }
${options.inject(contentsPlaceholder, digestPlaceholder)}
;;;
...
const setup = async (build, options) => {
  ...
  patchedBuild.onLoad({ filter: modulesCssRegExp }, async ({ path }) => {
    ...
    sortBy(Object.entries(result.exports ?? {}), '0').forEach(([origin, local]) => {
      ...
      // TODO: support named exports, maybe need to validate jsName is not a js key word
      // jsLines.push(`export const ${jsName} = "${localName}";`);

      jsNames.push([jsName, localName]);
      if (keepOrigin && origin !== jsName) {
        originNames.push([origin, localName]);
      }
    });
