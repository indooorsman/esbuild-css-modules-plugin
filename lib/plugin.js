...
const setup = async (build, options) => {
  ...
  patchedBuild.onLoad(
    ...
    ({ path, pluginData, namespace }) => {
      ...
      log(`on load ${namespace}:${path} in ${pluginData.resolveDir}`);
  ...
  patchedBuild.onResolve(
    ...
    ({ path, pluginData, resolveDir, namespace }) => {
      log(`on resolve ${namespace}:${path} in ${resolveDir}`);
      return {
        path,
  ...
  patchedBuild.onLoad(
    ...
    async ({ pluginData, path, namespace }) => {
      ...
      log(`on load ${namespace}:${path} in ${resolveDir}`);
...
${uniq([...jsNames, ...originNames])
  ...
  patchedBuild.onResolve(
    ...
    ({ path, resolveDir }) => {
      log(
        `resolve ${path} in ${resolveDir} to ${path}.${virtualJsExt} with namespace ${pluginJsNamespace}`
      );
      return {
